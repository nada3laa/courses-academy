...
export default function TeacherMessages() {
  ...
  useEffect(() => {
    ...
    if (!classroomId && !roomId && !userId) return;

    const conversation = conversations.find(
      (item) =>
        (roomId && String(item.id) === roomId) ||
        (classroomId && String(item.classroomId) === String(classroomId)) ||
        (userId && item.participants?.some((participant) => {
          const participantId = participant.id || participant._id || participant.user?.id || participant.user?._id || participant.user;
    ...
    if (conversation) {
      ...
      startSupportConversation(userId).then((newRoomId) => {
        if (newRoomId) setShowChatOnMobile(true);
        else openedFromLink.current = false;
      });
    }
  }, [conversations, loading, location.state, openConversation, searchParams, startSupportConversation]);
